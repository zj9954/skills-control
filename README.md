...
- **🔄 自动更新** — 记录每个技能的来源仓库与提交，一键检查/全部更新；可开启定时自动更新（应用运行期间按设定间隔静默检查并更新）
...
2. **检查更新**：「检查更新」会按来源仓库比对最新提交，有更新的技能行首会亮琥珀灯；也可以在「设置」里打开自动更新。
3. **同步**：技能行的 ⇄ 按钮可以把技能复制给其他 Agent。
4. **Token（可选）**：GitHub 未登录 API 有速率限制（约 60 次/小时），在「设置」填入 [Personal Access Token](https://github.com/settings/tokens) 可提升到 5000 次/小时，并支持私有仓库。
...
- 通过 GitHub API 解析仓库文件树定位 `SKILL.md`，用 tarball 一次性下载后按需提取，不需要本机安装 git
